...
<script>
import WLExplorer from "wl-explorer-view";
import "wl-explorer-view/lib/wl-explorer.css"
import request from "@/util/request";
import { Base64 } from "js-base64";
import axios from "axios";
import JSZip from "jszip";
import * as FileSaver from "file-saver";
// import { serviceHost } from "@/utils/globalConfig";
let serviceHost = ''
...
export default {
  ...
  components: {
    wlExplorer: WLExplorer.wlExplorer
  },
  ...
  methods: {
    ...
    getFileList() {
      ...
      request({
        url: `/api/sky/supportFile/dataGrid`,
        method: "post",
    ...
    search() {
      ...
      request({
        url: `/api/sky/supportFile/search`,
        method: "post",
    ...
    filePreview(data, cb) {
      ...
      if (data.type === 2) {
        ...
        // this.preview.url = `${this.host}/preview/onlinePreview?url=${encodeURIComponent(Base64.encode(path))}`
        this.preview.url = `https://xx.xx.xx.xx:7891/preview/onlinePreview?url=${encodeURIComponent(Base64.encode(path))}`
        this.preview.type = 'iframe'
        cb()
      } else {
